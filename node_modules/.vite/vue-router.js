import "./chunk-4DDRMBBZ.js";
import "./chunk-S6WPKAEG.js";
import {
  computed,
  defineComponent,
  getCurrentInstance,
  ...
  unref,
  warn
} from "./chunk-HJV52JH5.js";
import "./chunk-ZAZH7SVW.js";
...
function scrollToPosition(position) {
  ...
  if ("el" in position) {
    ...
    if (!el) {
      warn2(`Couldn't find element using selector "${position.el}"`);
      return;
    }
...
function useHistoryStateNavigation(base) {
  ...
  function push(to, data) {
    const currentState = assign({}, history2.state, {
      forward: to,
...
function createRouterMatcher(routes, globalOptions) {
  ...
  function resolve(location2, currentLocation) {
    ...
    if ("name" in location2 && location2.name) {
      ...
      params = assign(paramsFromLocation(currentLocation.params, matcher.keys.map((k) => k.name)), location2.params);
...
function onBeforeRouteLeave(leaveGuard) {
  ...
  activeRecord.leaveGuards.push(leaveGuard.bind(instance.proxy));
}
function onBeforeRouteUpdate(updateGuard) {
  ...
  activeRecord.updateGuards.push(updateGuard.bind(instance.proxy));
}
function guardToPromiseFn(guard, to, from, record, name) {
  ...
  return () => new Promise((resolve, reject) => {
    ...
    let guardCall = Promise.resolve(guard.call(record && record.instances[name], to, from, true ? canOnlyBeCalledOnce(next, to, from) : next));
    if (guard.length < 3)
      guardCall.then(next);
...
var RouterViewImpl = defineComponent({
  ...
  setup(props, { attrs, slots }) {
    ...
    return () => {
      ...
      return slots.default ? slots.default({ Component: component, route: matchedRoute }) : component;
    };
  }
});
...
${JSON.stringify(newTargetLocation, null, 2)}
 when navigating to "${targetLocation.fullPath}". A redirect must contain a name or path. This will break in production.`);
      ...
      return pushWithRedirect(assign({}, targetLocation, newTargetLocation, {
        state: data,
        force,
        replace: replace2
  ...
  function navigate(to, from) {
    ...
    return runGuardQueue(guards).then(() => {
      ...
      guards = extractComponentsGuards(to.matched.filter((record) => from.matched.indexOf(record) < 0), "beforeRouteEnter", to, from);
      guards.push(canceledNavigationCheck);
      return runGuardQueue(guards);
    }).then(() => {
      guards = [];
